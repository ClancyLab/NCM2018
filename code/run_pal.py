...
def submit_job(run):
    ...
    py_script = '''
from run_pal import run_replication
import cPickle as pickle
...
def run_pal(dataset, stats, NUM_RUNS=1000, parallel=True, on_queue=False):
    START = 1
    NUM_REOPT = 10
    ...
    pickle.dump(dataset["PAL"], open("out/d_pal.dat", 'wb'))
    pickle.dump(each_run, open("out/best_pal.dat", 'wb'))

    stats["PAL"] = [n_to_max, N_POINTS]
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    dataset, stats = run_pal({}, {}, NUM_RUNS=1, parallel=True, on_queue=False)
