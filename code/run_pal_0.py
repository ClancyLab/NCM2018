...
def run_replication(run, START, use_eps, use_rho, data, data_x2, data_x, data_y, data_sp, NUM_REOPT, parallel):
    ...
    for j in range(len(data_x) - START):
        if (not j % NUM_REOPT) or (len(x) - NUM_REOPT < 0):
            ...
            if parallel:
                hps = pal.MLE_parallel(x, y, sp)
            else:
                hps = pal.MLE(x, y, sp)

            # Set prior
            mu = np.array([4 * hps[0] + hps[3] for i in data_x])
            # print hps[-len(sp[0]):], sp[0], len(sp[0])
            cov = np.array(
                hps[1] * np.dot(data_x, data_x.T) +
                hps[2] * np.diag(np.ones(len(data_x))) +
                hps[4] +
                pal.mk52(data_sp, hps[-len(sp[0]):], hps[-(len(sp[0]) + 1)])
            )
...
def submit_job(run):
    ...
    py_script = '''
from run_pal_0 import run_replication
import cPickle as pickle
...
def run_pal_0(dataset, stats, NUM_RUNS=1000, parallel=True, on_queue=False):
    START = 10
    NUM_REOPT = 10
    ...
    for run in range(NUM_RUNS):
        if not on_queue:
            pal.printProgressBar(run, NUM_RUNS, prefix="Running PAL_0 case...")
    ...
    pickle.dump(dataset["PAL_0"], open("out/d_pal_0.dat", 'wb'))
    pickle.dump(each_run, open("out/best_pal_0.dat", 'wb'))

    stats["PAL_0"] = [n_to_max, N_POINTS]

    pal.printProgressBar(NUM_RUNS, NUM_RUNS, prefix="Running PAL_0 case...", suffix="Done")
    return dataset, stats


if __name__ == "__main__":
    print("Running PAL for only one replication! If you wish to do a full run, please use the run.py code.")
    t0 = time.time()
    dataset, stats = run_pal_0({}, {}, NUM_RUNS=1, parallel=True, on_queue=False)
