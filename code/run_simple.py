...
def submit_job(run):
    ...
    py_script = '''
from run_simple import run_replication
import cPickle as pickle
...
def run_simple(dataset, stats, NUM_RUNS=1000, parallel=True, on_queue=False):
    START = 1
    NUM_REOPT = 10
    ...
    pickle.dump(dataset["SIMPLE"], open("out/d_simple.dat", 'wb'))
    pickle.dump(each_run, open("out/best_simple.dat", 'wb'))
